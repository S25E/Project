...
namespace ProjectP3Reserveringsapplicatie
{
    ...
    public partial class ReserveringBeheer
    {
        ...
        public void AddReservering(Reservering reservering)
        {
            ...
            db.InsertData("INSERT INTO RESERVERING VALUES ('" + reservering.Reserveringsnummer + "', '" + "N')");

            //Persoon(hoofdboeker) toevoegen, altijd 1 hoofdboeker!
            db.InsertData("INSERT INTO PERSOON VALUES('" + reservering.Hoofdboeker.Nummer + "', '" + reservering.Hoofdboeker.Naam + "', 'hoofdboeker', '" + reservering.Hoofdboeker.Telefoonnummer + "', '" + reservering.Hoofdboeker.Wachtwoord + "', 'N')");

            //Personen(bijboeker) toevoegen, kunnen meerdere zijn!
            foreach (Persoon p in reservering.Bijboekers)
            {
            ...
            if (reservering.Materialen.Count > 0)
            {
                foreach (Materiaal m in reservering.Materialen)
                {
        ...
        {
            int HBpersoonnummer = Convert.ToInt32(db.SelectData("SELECT HB.PERSOON_NUMMER FROM HOOFDBOEKER HB, PERSOON PE WHERE HB.PERSOON_NUMMER = PE.PERSOON_NUMMER AND HB.RESERVERING_NUMMER ='" + reservering.Reserveringsnummer + "'"));
            DataTable dt = db.getData("SELECT BB.PERSOON_NUMMER as persoon_nummer FROM BIJBOEKER BB, PERSOON PE WHERE BB.PERSOON_NUMMER = PE.PERSOON_NUMMER AND BB.RESERVERING_NUMMER ='" + reservering.Reserveringsnummer + "'");
            ...
            db.SelectData("DELETE FROM BIJBOEKER WHERE RESERVERING_NUMMER = '" + reservering.Reserveringsnummer + "'");
            db.SelectData("DELETE FROM HOOFDBOEKER WHERE RESERVERING_NUMMER = '" + reservering.Reserveringsnummer + "'");
            db.SelectData("DELETE FROM RESERVERING WHERE RESERVERING_NUMMER = '" + reservering.Reserveringsnummer + "'");
            ...
            db.SelectData("UPDATE CAMPINGPLAATS SET RESERVERING_NUMMER = 0 WHERE RESERVERING_NUMMER ='" + reservering.Reserveringsnummer + "'");
        ...
        {
            DataTable dt = db.getData("Select RESERVERING_NUMMER, BETAALD FROM RESERVERING");
            int Reserveringsnummer;
            string betaald;
            foreach (DataRow rdr in dt.Rows)
            {
                Reserveringsnummer = Convert.ToInt32(rdr["RESERVERING_NUMMMER"]);
